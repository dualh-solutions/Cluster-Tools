...
export default function ExtractPdfPagesTool() {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        if (pagesToExtract.length === 0) {
          updateFile(f.id, { status: "error", errorMessage: "No valid pages to extract." });
          continue;
        }
        ...
        console.error(err);
        updateFile(f.id, { status: "error", errorMessage: "Failed to extract PDF pages." });
      }
    }
  ...
  const handleDownloadAll = async () => {
    ...
    if (doneFiles.length === 1) {
      ...
      downloadResult(doneFiles[0].resultBlob!, `${baseName}_extracted.pdf`);
      return;
    }
    ...
    if (doneFiles.length > 1) {
      ...
      doneFiles.forEach(f => {
        ...
        zip.file(`${baseName}_extracted.pdf`, f.resultBlob!);
      });
      const content = await zip.generateAsync({ type: "blob" });
      downloadResult(content, `extracted_pdfs.zip`);
    }
  };
  ...
  return (
    <ToolLayout
      ...
      description="Extract specific pages from your PDF documents entirely in your browser."
    >
      {files.length === 0 && (
        <Dropzone
        ...
        <div className="w-full bg-surface border border-border rounded-[var(--radius-lg)] p-6 md:p-8 mt-8 shadow-sm animate-in fade-in slide-in-from-bottom-4 duration-300">
          ...
          {!isComplete && (
            <div className="bg-canvas rounded-[var(--radius-md)] p-6 mb-8 border border-border">
              <div>
                <label className="block text-sm font-medium text-ink mb-2">Pages to Extract</label>
                <input 
                  type="text" 
                  value={pageRanges}
                ...
                />
                <p className="text-xs text-ink-muted mt-2">Enter page numbers and/or ranges separated by commas.</p>
              </div>
            </div>
          )}
