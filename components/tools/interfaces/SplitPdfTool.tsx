...
export default function SplitPdfTool() {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        for (let pageIndex = 0; pageIndex < totalPages; pageIndex++) {
          ...
          const paddedNum = (pageIndex + 1).toString().padStart(padLength, '0');
        ...
        console.error(err);
        updateFile(f.id, { status: "error", errorMessage: "Failed to split PDF. It might be corrupted or encrypted." });
      }
    }
  ...
  const handleDownloadAll = async () => {
    ...
    if (doneFiles.length === 1) {
      ...
      downloadResult(doneFiles[0].resultBlob!, `${baseName}_split.zip`);
      return;
    }
    ...
    if (doneFiles.length > 1) {
      ...
      doneFiles.forEach(f => {
        const baseName = f.file.name.replace(/\.[^/.]+$/, "");
        zip.file(`${baseName}_split.zip`, f.resultBlob!);
      });
      const content = await zip.generateAsync({ type: "blob" });
      downloadResult(content, `split_pdfs.zip`);
    }
  };
  ...
  return (
    <ToolLayout
      ...
      description="Split PDF files into individual pages entirely in your browser. Fast, secure, and private."
    >
      {files.length === 0 && (
        <Dropzone
